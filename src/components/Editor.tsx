...
interface EditorProps {
  ...
  code: string;
  hasFile: boolean;
  onCodeChange: (code: string, hasFile: boolean) => void;
}
...
function Editor({ isRunning, code, onCodeChange, hasFile }: EditorProps) {
  ...
  // keep refs up-to-date so the editor's internal listener can call latest props
  useEffect(() => {
    onCodeChangeRef.current = onCodeChange;
    hasFileRef.current = hasFile;
  }, [onCodeChange, hasFile]);
        ...
        EditorView.updateListener.of((update) => {
          if (update.docChanged) {
            if (!suppressChangeRef.current) {
              onCodeChangeRef.current(update.state.doc.toString(), hasFileRef.current);
            }
          }
        }),
      ],
  ...
  useEffect(() => {
    ...
    // Sync code if changed externally. Suppress update listener during programmatic changes
    if (view.state.doc.toString() !== code) {
      ...
      // allow the next user-driven change to propagate
