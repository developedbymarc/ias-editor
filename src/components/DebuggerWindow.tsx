...
function DebuggerWindow({
  ...
  return (
    <div className="debugger-container" style={{ display: "grid", gridTemplateRows: `${topPct}% 2px ${100 - topPct}%`, gap: "2px", height: "100%" }}>
            ...
            <tbody>
              {Object.entries(cpuState.REGISTERS).map(([name, reg]) => (
                <tr key={name}>
                  ...
                  <td className="reg-instr">{reg.instr ? reg.instr : "-"}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <Splitter
        ...
        containerSelector=".debugger-container"
        getStartPct={() => topPct}
        onResize={(p) => setTopPct(p)}
        minPct={10}
        maxPct={90}
      />
